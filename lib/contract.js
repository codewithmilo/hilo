...
const setupGameEvents = (provider, account, updateFn) => {
  const HILOContract = new Contract(
    CONSTANTS.HILO_CONTRACT_ADDRESS,
    abi.abi,
    provider
  ...
  HILOContract.on(filter, (player, tokenId, event) => {
    console.log("buyPriceUpdate:", player, tokenId.toNumber(), event);
  ...
  HILOContract.on(filter, (player, tokenId, event) => {
    console.log("priceUpdated:", player, tokenId.toNumber(), event);
